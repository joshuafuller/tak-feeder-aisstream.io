...
const typeFilter = (typeof process.env.TYPE_FILTER !== 'undefined') ? process.env.TYPE_FILTER.split(",") : [35];
...
module.exports.ais2cot = (item,stale) => {
    const dt = Date.now();
    const dtD = new Date(dt).toISOString();
    const dtDs = new Date(dt + (10 * stale * 1000)).toISOString();
    if ((item.hasOwnProperty('lat')) && (item.hasOwnProperty('lon'))) {
        let mmsi = item["mmsi"].toString();
        let cottype = "a-";
        let affil = getAffil(mmsi);
        let typesuffix = (typedb.hasOwnProperty(item.type)) ? typedb[item.type] : "-S-X";
        cottype += affil[0];
        cottype += typesuffix;
        country = affil[1];
        ...
        if (typeof(item["type"]) !== "undefined") {
            ...
            if (typeFilter.includes(aisType)) return result;
        }
    }
}
